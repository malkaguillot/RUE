...
dir_path = current_path.replace("reforms_irpp\\reforms_irpp", "RUE\\")
...
#reform_fullsystem
#########################
print_bracket_most_recent_params(tbs_copy, 1)


#def create_reform_modif_taux_irpp(tranche = None, new_taux = None, year = None):
#    """
#    Fonction permettant de créer une réforme simple qui modifie le taux de l'IR d'une tranche donnée à une année donnée
#    """
#    assert tranche is not None
#    assert new_taux is not None
...
#    return modif_taux_irpp
#
#modif_taux_irpp = create_reform_modif_taux_irpp(tranche = 1, new_taux = 0.01, year = pre_reform_year)
#
#tax_benefit_systems_dict[reform_year] = modif_taux_irpp(tax_benefit_systems_dict[reform_year])
#print_bracket_most_recent_params(tax_benefit_systems_dict[reform_year], 1)


###############################################
# Essai avec le changement d'un taux seulement
###############################################
#
#
#def update_rates(parameters):
#    year = pre_reform_year
#    reform_period = periods.period(str(year))
#    # Reform
#    parameters.impot_revenu.bareme.brackets[0].rate.update(start = reform_period, value = 0.04)
#    return parameters
#
#class update_irpp(Reform):
#    name = "explicit_title"
#    def apply(self):
#        self.modify_parameters(modifier_function = update_rates)
#
#tax_benefit_systems_dict[reform_year] = update_irpp(tbs_copy)
#print_bracket_most_recent_params(tax_benefit_systems_dict[reform_year], 1)
#print_bracket_most_recent_params(tax_benefit_systems_dict[reform_year], 0)
...
class reform_fi(Reform):
    name = 'Proposition France Insoumise'
    def apply(self):
        self.modify_parameters(modifier_function = modify_parameters)
tbs_reformed = reform_fi(tbs_copy)
tax_benefit_systems_dict[reform_year] = reform_fullsystem(tbs_reformed)
print_bracket_most_recent_params(tax_benefit_systems_dict[reform_year], 8)


# Exploration a la main
#file_path = os.path.join(dir_path, 'irpp_lfi.yaml')
#reform_parameters_subtree = load_parameter_file(name='irpp_lfi', file_path=file_path)
#tax_benefit_systems_dict[reform_year].parameters.impot_revenu.bareme = reform_parameters_subtree
#tax_benefit_systems_dict[reform_year].parameters.impot_revenu.bareme
#tax_benefit_systems_dict[reform_year].parameters.impot_revenu.bareme.brackets[1]
